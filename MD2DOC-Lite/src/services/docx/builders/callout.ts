...
export const createCallout = async (content: string, type: BlockType, docxConfig?: DocxConfig): Promise<Paragraph> => {
  const calloutConfigs: Partial<Record<BlockType, { label: string; color: string; style: (typeof BorderStyle)[keyof typeof BorderStyle]; size: number; bg: string }>> = {
  ...
  const config = calloutConfigs[type] || { label: "NOTE", color: COLORS.CALLOUT.NOTE.BORDER, style: BorderStyle.DASHED, size: LAYOUT.BORDER.CALLOUT_NOTE, bg: COLORS.CALLOUT.NOTE.BG };
